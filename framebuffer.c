...
error_t framebuffer_init() {
  mailbox_buffer[0] = 35 * 4;     // buffer size in bytes
  mailbox_buffer[1] = 0x00000000; // request

  // Set physical size
  mailbox_buffer[2] = 0x00048003; // tag ID
  mailbox_buffer[3] = 8;          // data size (2 × 4 bytes)
  mailbox_buffer[4] = 0;          // request code
  mailbox_buffer[5] = 1024;       // width
  mailbox_buffer[6] = 600;        // height
  ...
  mailbox_buffer[13] = 4;          // data size (1 × 4 bytes)
  mailbox_buffer[14] = 0;
  mailbox_buffer[15] = 32; // 32 bits per pixel
  ...
  mailbox_buffer[20] = 0;  // the GPU will write the size here
